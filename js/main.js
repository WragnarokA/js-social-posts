...
for (let i = 0; i < buttons.length; i++) {
    ...
    currentBtn.addEventListener("click", function(e){
        ...
        if (!currentBtn.classList.contains(clickedClass)) {
            ...
            let likes = parseInt(contatori[i].innerText) +1;
            likes++;
...
function renderPost(post){
    ...
    const {author, likes, created, media, content} = post;

    if (post.author.image != null) {
        imageTemplate = `<img class="profile-pic" src="${post.author.image}" alt="${post.author.name}"></img>`
    } else {
        imageTemplate = `
        <div class="profile-pic-default">
            <samp>${ getIniziali(post.author.name)}</samp>
        </div>`
        ...
        <div class="post__header">
            <div class="post-meta">                    
                ...
                <div class="post-meta__data">
                    ...
                    <div class="post-meta__time">${post.created}a</div>
                </div>                    
            </div>
        </div>
        ...
        <div class="post__footer">
            <div class="likes js-likes">
                ...
                <div class="likes__counter">
                    Piace a <b id="like-coun ter-1" class="js-likes-counter">${post.likes}</b> persone
                </div>
            </div> 
        </div>            
    </div>`;
...
function getIniziali(nome){
    return nome[0] + nome.split(" ")[1][0];
